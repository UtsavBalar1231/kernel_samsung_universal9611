...
conn_gadget_function_unbind(struct usb_configuration *c, struct usb_function *f)
{
	...
	if (conn_gadget_lock(&dev->ep_out_excl)) {
		...
		wait_event(dev->unbind_wq, (atomic_read(&dev->ep_out_excl) == 0));
